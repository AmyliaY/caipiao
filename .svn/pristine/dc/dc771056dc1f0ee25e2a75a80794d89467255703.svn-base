	...
	<article class="page-container">
		<form class="form form-horizontal" id="form-qiuhao-add">
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-4 col-sm-4">
					<input type="text" name="phone"
						class="input-text" value="${userschongzhijilu.users.phone }">
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-4"><span
				...
				<div class="formControls col-xs-4 col-sm-4">
					<input type="text" name="money"
						class="input-text" value="${userschongzhijilu.money }">
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-4"><span
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-4 col-sm-4">
					<select type="text" name="dingdanid"
						class="input-text" id="dingdanid">
					</select>
				</div>
			</div>
			<div class="row cl">
...
$(function(){
	$("#form-qiuhao-add").validate({
		...
		submitHandler:function(form){
			$(form).ajaxSubmit({
				type: 'post',
				url: "${pageContext.request.contextPath}/chongzhirecord.do?p=bianji" ,
...
window.onload=function()
{
	...
	var type = "${userschongzhijilu.type}";
	$("#type").val(type)
	var status = "${userschongzhijilu.status}";
	$("#status").val(status)
	$.ajax({
		type:'post',
		url:'${pageContext.request.contextPath}/chongzhirecord.do?p=getdingdanid',
		success:function(data)
		{
			var json = eval(data);
