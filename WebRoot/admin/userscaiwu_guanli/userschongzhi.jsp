...
<body>
	...
	用户充值 <a class="btn btn-success radius r mr-20"
					...
					<c:forEach items="${map.list}" var="a" varStatus="i">
						<tr class="text-c">
							...
							<td><c:if test="${a.status==0}">
									<span class="btn btn-default radius">未付款</span>
								</c:if> <c:if test="${a.status==1}">
									<span class="btn btn-success radius">已付款</span>
								</c:if></td>
							<td>${a.time}</td>
							<td><c:if test="${a.type==1}">
									<span class="btn btn-success radius">微信</span>
								</c:if> <c:if test="${a.type==2}">
									<span class="btn btn-success radius">支付宝</span>
								</c:if> <c:if test="${a.type==3}">
									<span class="btn btn-success radius">银行卡</span>
								</c:if></td>
							<td><a href="javascript:;"
								onclick="userschongzhi_update('用户充值信息修改','${pageContext.request.contextPath}/userscaiwu.do?p=userschongzhi_bianji&id=${a.id }','800','550')">编辑</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
						...
						<table width="100%" border="0" align="center" cellpadding="0"
							...
							<tr>
								...
								<td width="32%" align="right" id="xiugaitype">[ <a
									href="${pageContext.request.contextPath}/userscaiwu.do?p=userschongzhi"
									class="right-font08">首页</a> | <a
									href="${pageContext.request.contextPath}/userscaiwu.do?p=userschongzhi&page=${map.page-1}"
									class="right-font08">上一页</a> | <a
									href="${pageContext.request.contextPath}/userscaiwu.do?p=userschongzhi&page=${map.page+1}"
									class="right-font08">下一页</a> | <a
									href="${pageContext.request.contextPath}/userscaiwu.do?p=userschongzhi&page=${map.pagecount}"
...
function viewGo()
{
		...
		if(page.match('^[0-9]*$'))
		{	
			window.location.href="${pageContext.request.contextPath}/userscaiwu.do?p=userschongzhi&page="+page;
		}
}
