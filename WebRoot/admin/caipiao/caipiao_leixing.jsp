				...
				<table id="tab"
					...
					<tbody>
						<c:forEach items="${caipiaomap.list}" var="a" varStatus="i">
							<tr class="text-c">
								<td>
									${i.count+(caipiaomap.page-1)*caipiaomap.size}
								</td>
								<td>
									${a.mingcheng}
								</td>
								<td>
								<c:if test="${a.status==0}"><span class="label label-default radius" >否</span></c:if>
								<c:if test="${a.status==1}"><span class="label label-success radius" >是</span></c:if>
								</td>
								<td>
									<a href="${pageContext.request.contextPath}/caipiaodian.do?p=caipiao_leixinglistingid&id=${a.id}" class="label label-success radius">
									编辑
									</a>
								</td>
							</tr>
