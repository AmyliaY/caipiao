...
public class AppCaipiaodianCenterService {
	...
	public String gotoOperationManager(Caipiaodian caipiaodian, boolean flag) {//flag，用来标记登陆的用户是商家还是店员，true指商家，false指店员
		...
		//订单数,2商家已接单 和 3订单完成
		String hql = "select SUM(*) from Dingdan where (status=2 or status=3) and caipiaodianByCpzid.id=?";
		String sumOrder = (int)hqlDAO.unique(hql, caipiaodian.getId())+"";
		//营业额：根据该彩票店完成的订单来算，3订单完成
		String hql2 = "select SUM(money) from Dingdan where status=3 and caipiaodianByCpzid.id=?";
	...
	public void shopConfirmation(Caipiaodianshenqing caipiaodianshenqing, String faren, String shenfenzheng, String sfzFrontImg, 
			...
			Double jingdu,Double weidu, Boolean isTicai, Boolean isFucai) {
		...
		caipiaodianshenqing.setJingdu(jingdu);
		caipiaodianshenqing.setWeidu(weidu);
		caipiaodianshenqing.setIsticai(isTicai);
		caipiaodianshenqing.setIsfucai(isFucai);
		//完成商户验证后才将Caipiaodianshenqing表status设置为0
